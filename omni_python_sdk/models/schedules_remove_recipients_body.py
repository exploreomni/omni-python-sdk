...
T = TypeVar("T", bound="SchedulesRemoveRecipientsBody")


@_attrs_define
class SchedulesRemoveRecipientsBody:
    """
    Attributes:
        emails (list[str] | Unset): At least one email, userId, or userGroupId must be provided. Array of recipient
            email addresses to remove from the scheduled task. Example: ['user@example.com'].
        user_group_ids (list[UUID] | Unset): At least one email, userId, or userGroupId must be provided. Array of user
            group UUIDs to remove as recipients. Example: ['123e4567-e89b-12d3-a456-426614174000'].
        user_ids (list[UUID] | Unset): At least one email, userId, or userGroupId must be provided. Array of recipient
            user UUIDs to remove from the scheduled task. Use the List users and List embed users endpoints to retrieve user
            IDs. Example: ['987fcdeb-51a2-43d7-9b56-254415f67890'].
    """
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        ...
        schedules_remove_recipients_body = cls(
            emails=emails,
            user_group_ids=user_group_ids,
            user_ids=user_ids,
        )

        schedules_remove_recipients_body.additional_properties = d
        return schedules_remove_recipients_body
